...
-- Create policies for user_profiles (with existence checks)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Create policies for study_sessions (with existence checks)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
...
-- Create policies for saved_questions (with existence checks)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'saved_questions' 
    AND policyname = 'Users can view own saved questions'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'saved_questions' 
    AND policyname = 'Users can insert own saved questions'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'saved_questions' 
    AND policyname = 'Users can update own saved questions'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'saved_questions' 
    AND policyname = 'Users can delete own saved questions'
  ) THEN
...
-- Create trigger for user_profiles (with existence check)
DO $$
BEGIN
  IF NOT EXISTS (
