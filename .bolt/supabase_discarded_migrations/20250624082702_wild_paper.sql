/*
  ...
  This migration adds all the missing RPC functions that the application needs.
  It includes proper error handling and ensures all functions work correctly.
*/

-- Drop existing functions if they exist to avoid conflicts
DROP FUNCTION IF EXISTS public.get_user_stats(uuid);
DROP FUNCTION IF EXISTS public.get_user_stats(user_id_param uuid);
DROP FUNCTION IF EXISTS public.refresh_user_profile_stats(uuid);
DROP FUNCTION IF EXISTS public.refresh_user_profile_stats(user_id_param uuid);
DROP FUNCTION IF EXISTS public.update_study_streak(uuid);
DROP FUNCTION IF EXISTS public.update_study_streak(user_id_param uuid);
DROP FUNCTION IF EXISTS public.increment_question_count(uuid);
DROP FUNCTION IF EXISTS public.increment_question_count(user_id_param uuid);
DROP FUNCTION IF EXISTS public.update_study_time(uuid, integer);
DROP FUNCTION IF EXISTS public.update_study_time(user_id_param uuid, duration_param integer);
DROP FUNCTION IF EXISTS public.calculate_study_streak(uuid);
DROP FUNCTION IF EXISTS public.calculate_study_streak(user_id_param uuid);

-- Create function to get user stats (matches frontend parameter name)
...
BEGIN
    ...
    IF NOT FOUND THEN
        RAISE NOTICE 'User profile not found for user_id: %', user_id_param;
    END IF;
END;
$$;
...
BEGIN
    -- Update study time
    PERFORM public.update_study_time(NEW.user_id, NEW.duration);
...
BEGIN
    ...
    PERFORM public.increment_question_count(NEW.user_id);
    
    RETURN NEW;
END;
$$;

-- Recreate triggers (drop existing ones first to avoid conflicts)
